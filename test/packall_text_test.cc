...
TEST(packall_lua, Config)
{
	...
	Config new_c;
	EXPECT_EQ(packall::lua::parse(new_c, kStr), packall::status::ok);
	...
	printf("%s\n", packall::lua::prettyprint(text).c_str());

	// Check that parsing the formatted version works correctly
	Config text_c;
	EXPECT_EQ(packall::lua::parse(text_c, text), packall::status::ok);
...
TEST(packall_lua, no_members)
{
	...
	std::string text;
	packall::lua::format(pt, text);

	printf("%s\n", packall::lua::prettyprint(text).c_str());

	point pt2{};
	packall::lua::parse(pt2, text);

	EXPECT_EQ(pt2, pt);
}

TEST(packall_json, Config)
{
	static const char kStr[] = R"({
	"device": "/dev/video0",
	"resolution": [640, 480],
	...
	// Check that parsing the known good version matches hardcoding
	Config new_c;
	EXPECT_EQ(packall::json::parse(new_c, kStr), packall::status::ok);
	EXPECT_EQ(new_c.device, c.device);
	EXPECT_EQ(new_c.resolution, c.resolution);
	EXPECT_EQ(new_c.K_matrix, c.K_matrix);
	EXPECT_EQ(new_c.distortion_coeffients, c.distortion_coeffients);
	...
	std::string text;
	packall::json::format(c, text);

	printf("%s\n", packall::json::prettyprint(text).c_str());

	// Check that parsing the formatted version works correctly
	Config text_c;
	EXPECT_EQ(packall::json::parse(text_c, text), packall::status::ok);
	EXPECT_EQ(text_c.device, c.device);
	EXPECT_EQ(text_c.resolution, c.resolution);
	EXPECT_EQ(text_c.K_matrix, c.K_matrix);
	EXPECT_EQ(text_c.distortion_coeffients, c.distortion_coeffients);
